...
try{  
  if(!isAuthenticate){ 
const userInput : Answers = await inquirer.prompt(loginQuestions) ;
...
if(userInput.accountNumber == userAccountNum && userInput.PIN == userPIN){
  ...
  let performAction : Answers = await inquirer .prompt([{
    name : "accountInfo" ,
    type : "list" ,
    ...
    if(performAction.accountInfo == "check account balance"){
      console.log(`Your account balance is ${accountBalance}`);
       ...
       let withdraw : Answers = await inquirer.prompt([{
        name : "moneyWithDraw" ,
        type : "number" ,
      ...
      console.log(`All num of previous transaction was: "${index}" and transaction amount was "${transactions}"`) ;
  ...
  console.log("Input is not valid or incorrect!");
   ...
   let performAnotherTransaction : Answers = await inquirer.prompt([{
    name : "performAnother" ,
    type : "confirm" ,
